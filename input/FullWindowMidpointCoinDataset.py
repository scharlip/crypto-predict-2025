...
class FullWindowMidpointCoinDataset(CoinDataset, Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.use_normalized_data:
            ...
            y = self.df.iloc[item + self.lookback + 1: item + self.lookahead + self.lookback + 1]["NormalizedMidpoint"].tolist()
        else:
            X = self.df[item : item + self.lookback]["Midpoint"].tolist()
            y = self.df.iloc[item + self.lookback + 1: item + self.lookahead + self.lookback + 1]["Midpoint"].tolist()
